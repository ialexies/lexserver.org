        ...
        <Reveal stagger className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {SERVICES.map((s) => (
            <RevealItem key={s.title}>
              <article className="card-shimmer group flex h-full flex-col border border-[#1e2d45] bg-[#111827] p-7 transition-all duration-200 hover:-translate-y-1 hover:border-pop hover:shadow-brut">
                ...
                <p className="mt-3 flex-1 text-[15px] leading-relaxed text-[#7890a8]">
                  {s.body}
                </p>
                <ul className="mt-5 flex flex-wrap gap-2">
                  {s.points.map((p) => (
                    <li
                      key={p}
                      className="border border-[#1e2d45] px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-[#7890a8]"
                    >
                      {p}
                    </li>
                  ))}
